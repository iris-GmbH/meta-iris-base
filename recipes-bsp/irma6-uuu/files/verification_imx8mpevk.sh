...
sha256sum /mnt/fat/imx8mp-evk.dtb | cut -d' ' -f1 | grep $HASH_DEVTREE || { echo "Error: Failed to verify hash of *.dtb on /dev/mmcblk"${mmc}"p2"; exit 1; }
umount /mnt/fat
sync
...
sha256sum /mnt/fat/imx8mp-evk.dtb | cut -d' ' -f1 | grep $HASH_DEVTREE || { echo "Error: Failed to verify hash of *.dtb on /dev/mmcblk"${mmc}"p3"; exit 1; }
umount /mnt/fat
sync
